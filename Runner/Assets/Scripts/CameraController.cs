...
namespace UltimateCameraController.Cameras.Controllers
{
	...
	public class CameraController : MonoBehaviour
	{
		...
		public void DestinationPlayer()
        {
			...
			_cameraOffset = /*transform.position - targetObject.position*/ new Vector3(0,10,-16);
		}
		//We use late update so that player movement is completed before we move the camera
		//This way we can avoid glitches
		private void LateUpdate()
		{
			//_cameraOffset = (transform.position - targetObject.position)+offset;
			...
			if (followTargetPosition)
			{
				...
				var newPosition = targetObject.position + _cameraOffset+offset;
